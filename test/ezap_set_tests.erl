...
-module(ezap_set_tests).
-ifdef(TEST).
-include_lib("eunit/include/eunit.hrl").
-include("include/ezap.hrl").

root_test() ->
    {ok, Root} = ezap_set:root('Test', test_zap),
    Data = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
    ?assertEqual(
...
text_field_test() ->
    {ok, Root} = ezap_set:root('Test', test_zap),
    ok = ezap:set(textField, <<"test data">>, Root),
    Data = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
    ?assertEqual(
       [<<0:32/integer-little, 2:16/integer-little, 6:16/integer-little,
          %% data
...
list_field_test() ->
    {ok, Root} = ezap_set:root('ListTest', test_zap),
    ?assertEqual([0,0,0], ezap:set(listInts, 3, Root)),
    ok = ezap:set(listInts, {1, 222}, Root),
    ok = ezap:set(listInts, {0, 111}, Root),
    ok = ezap:set(listInts, {2, -333}, Root),
    Data = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
    ?assertEqual(
...
object_field_test() ->
    {ok, Root} = ezap_set:root('ListTest', test_zap),
    ?assertEqual([false, false], ezap:set(listAny, {{list, bool}, 2}, Root)),
    ok = ezap:set(listAny, {{list, bool}, {1, true}}, Root),
    ok = ezap:set(listAny, {{list, bool}, {0, false}}, Root),
    Data = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
    ?assertEqual(
       [<<0,0,0,0, 0,0,4,0, %% struct ref off 0, 0 data, 4 ptrs
          %% pointers
...
object_as_struct_test() ->
    {ok, Root} = ezap_set:root('ListTest', test_zap),
    Obj = ezap:init(listAny, 'Simple', Root),
    ok = ezap:set(simpleMessage, <<"object text">>, Obj),
    Data = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
    ?assertEqual(
       [<<0,0,0,0, 0,0,4,0, %% struct ref off 0, 0 data, 4 ptrs
          %% pointers
...
struct_list_test() ->
    {ok, Root} = ezap_set:root('ListTest', test_zap),
    [R1, R2] = ezap:set(listSimples, 2, Root),
    ok = ezap:set(value, 332211, R1),
    ok = ezap:set(defaultValue, 112233, R2),
    Data = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
    ?assertEqual(
       [<<0,0,0,0, 0,0,4,0,
...
text_list_test() ->
    ...
    {ok, Root} = ezap:set_root('ListTest', test_zap),
    ezap:set(listText, 3, Root),
    ezap:set(listText, {0, Text1}, Root),
    ezap:set(listText, {1, Text2}, Root),
    ezap:set(listText, {2, Text3}, Root),
    Data = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
...
set_struct_by_props_test() ->
    {ok, Root} = ezap_set:root('Test', test_zap),
    {Obj, [ok]} = ezap:set(structField, [{simpleMessage, <<"object text">>}], Root),
...
set_cap_test() ->
    {ok, Root} = ezap:set_root('CapTest', test_zap),
    ...
    ok = ezap:set(obj, {{interface, 'BasicCap'}, Cap1}, Root),
    ok = ezap:set(basic, Cap2, Root),
...
init_union_test() ->
    {ok, Root} = ezap:set_root('UnionTest', test_zap),
    Empty = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
    ?assertEqual(
      [<<0:32/integer-little, 1:16/integer-little, 1:16/integer-little,
         0:2/integer-unit:64>>], Empty),

    Null = ezap:set(test, Root),
    ?assertEqual('Test', Null#object.schema#schema_node.name),
    ?assertEqual(null, Null#object.ref#ref.kind),
    Test = ezap_obj:init(Null),
    ok = ezap:set(intField, 66, Test),
...
init_union_by_props_test() ->
    ...
    {Test, [ok]} = ezap:set({test, [{intField, 66}]}, Root),
    ?assertEqual({test, Test}, ezap:get(Root)),
...
init_union_any_test() ->
    ...
    Obj = ezap:init(any, 'Simple', Root),
    ?assertEqual('Simple', Obj#object.schema#schema_node.name),
    ?assertEqual(#struct_ref{ dsize=1, psize=2 }, Obj#object.ref#ref.kind),
    ok = ezap:set(value, 111, Obj),
...
packed_struct_list_test() ->
    {ok, Root} = ezap:set_root('PackedListTest', test_zap),
    [O1, O2, O3] = ezap:set(packedList, 3, Root),
    ok = ezap:set(flag, true, O1),
    ok = ezap:set(toggle, true, O1),
    ok = ezap:set(flag, true, O2),
    ok = ezap:set(toggle, true, O3),
    ok = ezap:set(value, 123, O1),
    ok = ezap:set(value, 234, O2),
    ok = ezap:set(value, 345, O3),
    Data = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
    ?assertEqual(
...
group_field_test() ->
    {ok, Root} = ezap:set_root('Test', test_zap),
    Grp = ezap:get(meta, Root),
    ok = ezap:set(id, 123, Grp),
    ok = ezap:set(tag, <<"foo bar">>, Grp),
    Data = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
    ?assertEqual(
...
named_union_test() ->
    ...
    {ok, Root} = ezap:set_root('Test', test_zap),
    ok = ezap:set(opts, {text, <<"testing">>}, Root),
    Data = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
    ?assertEqual(
      [<<0:32/integer, 2,0, 6,0, %% Test struct, 16 bytes, 6 ptrs
...
unnamed_union_field_test() ->
    {ok, Root} = ezap:set_root('Test', test_zap),
    ok = ezap:set({boolField, true}, Root),
    Data = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
    ?assertEqual(
      [<<0:32/integer, 2,0, 6,0, %% Test struct, 16 bytes, 6 ptrs
...
unnamed_union_group_test() ->
    {ok, Root} = ezap:set_root('Test', test_zap),
    Grp = ezap:set(groupField, Root),
    ok = ezap:set(a, 0, Grp),
    ok = ezap:set(c, 33, Grp),
    Data = ezap_data:get_segments((Root#object.ref)#ref.data#builder.pid),
    ?assertEqual(
      [<<0:32/integer, 2,0, 6,0, %% Test struct, 16 bytes, 6 ptrs
