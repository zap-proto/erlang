...
%% generate any kind of primitive value that zap can represent.
...
prop_zap_value() ->
    ?FORALL(
       {_S, T, V}, data_value(),
       compare_value(V, ezap_val:get(T, ezap_val:set(T, V)))).
...
prop_struct_data_builder() ->
    ?FORALL(
       ...
       begin
           ...
           ok = ezap_ref:write_struct_data(Off, Size, Value, Root),
           Value =:= ezap_ref:read_struct_data(Off, Size, Root)
       end).
...
prop_struct_data_reader() ->
    ?FORALL(
       ...
       begin
           ...
           %% note: value is binary
           {error, read_only} = ezap_ref:write_struct_data(Off, Size, Value, Root),
           <<0:Size/integer>> =:= ezap_ref:read_struct_data(Off, Size, Root)
       end).
...
prop_struct_ptr() ->
    ?FORALL(
       ...
       begin
           Root = data_builder(Def),
           #ref{ kind=null }=Ref = ezap_ref:read_struct_ptr(Idx, Root),
           ok = ezap_ref:write_struct_ptr(Ref#ref{ kind=Kind }, Root),
           case ezap_ref:read_struct_ptr(Idx, Root) of
               #ref{ kind=Kind } -> true;
               #ref{ kind=null }
...
prop_text_data() ->
    ?FORALL(
       ...
       begin
           ...
           Ref0 = ezap_ref:read_struct_ptr(Idx, Root),
           ok = ezap_ref:write_text(Text, Ref0, Root),
           Ref1 = ezap_ref:read_struct_ptr(Idx, Root),
           Text =:= ezap_ref:read_text(Ref1)
       end).
...
test_field_access({Schema, FVs}) ->
    ...
    Obj = ezap_obj:from_ref(Root, Schema, no_schema),
    lists:all(
      fun ({F, V}) ->
              ok = ezap:set(F, V, Obj),
              compare_value(V, ezap:get(F, Obj))
      end, FVs).
...
prop_find_schema() ->
    ?FORALL(
       ...
       begin
           ...
           ByModule = S =:= ezap_schema:lookup(IdOrType, schema_zap),
           %% find by schema from same schema file
           BySchema = S =:= ezap_schema:lookup(IdOrType, schema_zap:'Type'()),
           %% already got the schema..
           NoOp = S =:= ezap_schema:lookup(IdOrType, S),
           NoOp2 = S =:= ezap_schema:lookup(S, no_matter),
           ?WHENFAIL(
...
schema() ->
    union(
     [?LET({Id, _Type}, schema_types(), {Id, schema_zap:schema(Id)}),
      ?LET({_Id, Type}, schema_types(), {Type, schema_zap:schema(Type)})
     ]).

schema_types() ->
    union(proplists:get_value(types, schema_zap:module_info(attributes))).
...
data_builder(#struct_ref{ dsize=D, psize=P }=Kind, Extra) ->
    {ok, Pid} = ezap_data:start_link(1 + D + P + Extra),
    ezap_ref:alloc(Kind, 0, 1 + D + P, #builder{ pid = Pid });
data_builder(#schema_node{}=N, Extra) ->
    data_builder(ezap_schema:get_ref_kind(N), Extra).

data_reader(#struct_ref{ dsize=D, psize=P }=Kind) ->
    R = ezap_ref:create_ptr(0, Kind),
    E = <<0:(D + P)/integer-unit:64>>,
    ezap_ref:get(0, 0, #reader{ data = <<R/binary, E/binary>> });
data_reader(#schema_node{}=N) ->
    data_builder(ezap_schema:get_ref_kind(N)).
