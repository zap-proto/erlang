...
import_capability(Vat, ObjectId, Schema) ->
    #promise{
       owner = {ezap_vat, Vat},
       pid = ezap_vat:import_capability(Vat, ObjectId),
       schema = Schema
      }.
...
wait(#promise{ pid = Pid }=Promise, Time) ->
    {ok, Res} = ezap_promise:wait(Pid, Time),
...
dump(#rpc_call{ target = T, interface = I, method = M, params = P,
    ...
    io_lib:format("#rpc_call{\n"
                  ...
                  "\tresultSchema = ~s\n"
                  "}",
                  [ezap:dump(T), I, M, ezap:dump(P),
                   ezap:dump(R), ezap:dump(S)]).
...
do_request(MethodName, {Target, Schema}) ->
    {ok, Interface, Method} = ezap_schema:find_method_by_name(
                                MethodName, Schema),
    {ok, Msg} = ezap_set:root('Message', rpc_zap),
    Call = ezap:init(call, Msg),
    Payload = ezap:init(params, Call),
    Content = ezap:init(
                content, ezap_schema:lookup(
                           Method#method.paramType, Interface),
                Payload),
    ResultSchema = ezap_schema:get(Method#method.resultType, Interface),
    #rpc_call{
       target = Target,
...
transform([{getPointerField, Idx}|Ts], Obj) ->
    transform(Ts, ezap:get({ptr, Idx}, Obj)).
...
wait_result(undefined, Res) ->
    ...
    Res;
wait_result(Schema, Res) ->
    Obj = ezap_obj:to_struct(Schema, Res),
