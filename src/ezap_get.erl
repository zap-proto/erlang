...
root(Node, Segments) ->
    {ok, ezap_obj:from_data(Segments, Node)}.
...
transform_promise(#promise{ transform = Ts, schema = S } = P, Type, T) ->
    P#promise{ transform = [T|Ts], schema = ezap_schema:get(Type, S) }.
...
read_field(#data{ type=Type, align=Align, default=Default }=D, Object) ->
    case Type of
        ...
        Type ->
            case ezap_val:size(Type) of
                0 -> void;
                Size ->
                    ezap_val:get(
                      Type, ezap_ref:read_struct_data(
...
read_field(#ptr{ idx=Idx }=Ptr, #object{ ref = Ref }=Object) ->
    Obj = ezap_obj:init(
            ezap_ref:read_struct_ptr(Idx, Ref),
            Object),
    read_ptr(Ptr, Obj);
read_field(#group{ id=Type }, Object) ->
    case ezap_obj:to_struct(Type, Object) of
        #object{
          schema = #schema_node{
                      kind = #struct{
                                union_field = Union,
...
read_ptr(#ptr{ type=Type, default=Default }, #object{ ref = Ref }=Obj) ->
    case Type of
        text -> ezap_ref:read_text(Ref, Default);
        data -> ezap_ref:read_data(Ref, Default);
...
read_obj(Type, #object{ ref = #ref{ kind=null } }=Obj, Default) ->
    if is_binary(Default) ->
            ezap_obj:from_data(Default, Type, Obj)
    end;
read_obj(Type, Obj, _) ->
    ezap_obj:to_struct(Type, Obj).

read_list({struct, StructType}, Default, #object{ ref = Ref }=Obj) ->
    Schema = ezap_schema:get(StructType, Obj),
    case ezap_ref:read_list_refs(
           Ref, ezap_schema:get_ref_kind(Schema), undefined)
    of
        [] -> [];
        undefined ->
            if is_binary(Default) ->
                    ezap_obj:from_data(Default, {list, {struct, StructType}}, Obj);
               true -> Default
            end;
...
read_list(ElementType, Default, #object{ ref = Ref }=Obj) ->
    case ezap_ref:read_list(Ref, undefined) of
        undefined ->
            if is_binary(Default) ->
                    ezap_obj:from_data(
        ...
        Values ->
            case ElementType of
                {enum, EnumType} ->
                    [get_enum_value(
                       EnumType,
                       ezap_val:get(uint16, Data),
                       Obj)
                     || Data <- Values];
                _ when is_atom(ElementType) ->
                    [ezap_val:get(ElementType, Data)
                     || Data <- Values]
            end
    end.
