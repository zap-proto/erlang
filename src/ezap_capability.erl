...
%%--------------------------------------------------------------------
start(Args) ->
    gen_server:start(?MODULE, Args, ?EZAP_GEN_OPTS).

start_link(Args) ->
    gen_server:start_link(?MODULE, Args, ?EZAP_GEN_OPTS).
...
do_dispatch(#schema_node{ name = InterfaceName }=Interface,
    ...
    ParamSchema = ezap_schema:get(ParamType, Interface),
    ResultSchema = ezap_schema:get(ResultType, Interface),

    {Params, Results} =
        case Args of
            {Ps, undefined} ->
                {ok, Rs} = ezap_set:root(ResultSchema),
                {Ps, Rs};
            {Ps, Payload} ->
                Content = ezap:init(content, ResultSchema, Payload),
                {Ps, Content}
        end,
    ...
    {ok, CapState1} = apply(Mod, handle_call,
                            [InterfaceName, MethodName,
                             ezap_obj:to_struct(ParamSchema, Params),
                             Results, CapState0]),
