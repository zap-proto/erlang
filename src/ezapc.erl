...
%% @doc The Erlang Zap Compiler plugin.
%%
%% This module takes a <a
%% href="https://zap-proto.dev/docs/otherlang#how_to_write_compiler_plugins">`CodeGeneratorRequest'</a>
%% message and compiles the requested schema files into Erlang header
%% files.
%%
%% Thus, a `my_schema.zap' will be compiled to
%% `my_schema.zap.hrl', implementing `my_schema/N' functions for
%% reading and writing Zap messages using `my_schema'.
%%
%%
%% == Schema functions ==
%%
...
%% to the `my_schema.zap' example in the {@section Description} section).
...
compile_data(Data)
  when is_binary(Data) ->
    {ok, Message, <<>>} = ezap_message:read(Data),
...
compile_message(Message)
  when is_list(Message), is_binary(hd(Message)) ->
    {ok, Compiled} = ezap_compiler:compile(Message),
...
maybe_compile_ast(Compiled) ->
    case os:getenv("EZAP_NO_BEAM") of
...
maybe_load(Module, Filename, Bin) ->
    case os:getenv("EZAP_LOAD_BEAM") of
        false -> ok;
        _ ->
