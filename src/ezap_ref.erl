...
%% Which is almost everything in Zap :p.
...
alloc(SegmentId, Size, #builder{ pid = Pid }=Builder) ->
    {Id, Pos} = ezap_data:alloc(SegmentId, Size, Pid),
...
%% conformant with Zap specifications.
...
alloc_data(Size, #ref{ segment=SegmentId, data=#builder{ pid=Pid } }=Ref) ->
    {Seg, Pos} = ezap_data:alloc(SegmentId, Size, Pid),
    ...
    {Align, ElementSize} =
        case Size of
            ...
            pointer ->
                {ElementIdx * 64, 64};
            1 ->
                %% take special care for bitfields, as zap wants them in big endian order..!
...
read(SegmentId, Pos, Len, #builder{ pid = Pid }) ->
    ezap_data:get_segment(SegmentId, Pos, Len, Pid);
...
write(Bin, Offset, #ref{ segment=SegmentId, pos=Pos, data=#builder{ pid = Pid } }) ->
    ezap_data:update_segment({SegmentId, Pos + Offset}, Bin, Pid);
...
get_cap_idx(Cap, #builder{ pid = Pid }) ->
    ezap_data:get_cap_idx(Cap, Pid).

get_cap(Idx, #builder{ pid = Pid }) ->
    ezap_data:get_cap(Idx, Pid);
