...
%% Holds all the segments in a Zap message.
%%
...
%% The top-level schema node (for the .zap-file).
...
%% Note that in Zap, a word is 8 bytes (64 bits).
...
%% @see ezap_get:root/3
%% @see ezap_serialize:unpack/1
%% @see ezap_message:read/1
get_root(Type, Schema, Segments) ->
    ezap_get:root(Type, Schema, Segments).

get_root(Schema, Segments) ->
    ezap_get:root(Schema, Segments).
...
%%
%% To get the segment data out, call {@link ezap_message:write/1}.
%% @see ezap_set:root/2
set_root(Type, Schema) ->
    ezap_set:root(Type, Schema).

set_root(Schema) ->
    ezap_set:root(Schema).
...
%% void.
%% @see ezap_get:union/1
get(Object) ->
    ezap_get:union(Object).

-spec get(field_name(), object()) -> field_value().
%% @doc Read the field value of object.
%% @see ezap_get:field/2
get(Field, Object) ->
    ezap_get:field(Field, Object).

-spec set({field_name(), field_value()}|field_name(), object()) -> ok.
%% @doc Write union value to the unnamed union of object.
%% @see ezap_set:union/2
set(Values, Object) when is_list(Values) ->
    ezap_set:fields(Values, Object);
set(Value, Object) ->
    ezap_set:union(Value, Object).

-spec set(field_name(), field_value(), object()) -> ok.
%% @doc Write value to a field of object.
%% @see ezap_set:field/3
set(Field, Value, Object) ->
    ezap_set:field(Field, Value, Object).

init(Value, Object) ->
    ezap_obj:init(ezap_set:field(Value, Object)).

init(Field, Type, Object) ->
    ezap_obj:init(
      ezap_obj:to_struct(
        Type, ezap_set:field(Field, Object))).

-spec const(type_name()|type_id(), schema()) -> value().
%% @doc Get const value from schema.
const(Name, Schema) ->
    case ezap_schema:lookup(Name, Schema) of
        #schema_node{ kind=#const{ field=Field } } ->
            case Field of
                #data{ type=Type, default=Value } ->
                    ezap_val:get(Type, Value);
                #ptr{}=Ptr ->
                    ezap_get:ref_data(Ptr, #ref{ data=not_yet_implemented })
            end
    end.

import_capability(Vat, ObjectId, Schema) ->
    ezap_rpc:import_capability(Vat, ObjectId, Schema).

request(Name, Target) ->
    ezap_rpc:request(Name, Target).

send(Req) ->
    ezap_rpc:send(Req).

wait(Promise) ->
    ezap_rpc:wait(Promise, infinity).

wait(Promise, Time) ->
    ezap_rpc:wait(Promise, Time).

dump(Object) when is_record(Object, object) -> ezap_obj:dump(Object);
dump(Request) when is_record(Request, rpc_call) -> ezap_rpc:dump(Request);
dump(Schema) when is_record(Schema, schema_node) -> ezap_schema:dump(Schema);
