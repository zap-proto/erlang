...
init(#object{ ref = #ref{ kind = null } = Ref, schema = Schema }=Obj) ->
    Obj#object{
      ref = ezap_ref:paste(
              ezap_ref:create_ptr(
                0, ezap_schema:get_ref_kind(Schema)),
              Ref)
     }.
...
alloc(Node, SegmentId, Data) when is_pid(Data) ->
    init(
      ezap_ref:alloc(
        ezap_schema:get_ref_kind(Node),
        SegmentId, 1 + ezap_schema:size_of(Node),
        #builder{ pid = Data }),
      Node).
...
from_ref(#ref{ kind=Kind }=Ref, {list, _}=Type, Schema)
  when is_record(Kind, list_ref); Kind == null ->
    ezap_get:ref_data(Type, init(Ref, Schema), []);
...
from_data(Data, Type, Schema) ->
    Ref = ezap_ref:get(0, 0, #reader{ data = Data }),
    from_ref(Ref, Type, Schema).
...
field(Field, #object{ schema = Schema }) ->
    ezap_schema:find_field(Field, Schema).
...
%% @see ezap_ref:refresh/1
refresh(#object{ ref=Ref }=Object) ->
    Object#object{ ref=ezap_ref:refresh(Ref) }.
...
to_struct(Type, #object{ ref=#ref{ kind=Kind } }=Object)
  when Kind == null;
    ...
    T = ezap_schema:lookup(Type, Object),
...
to_list(Type, #object{ ref=#ref{ kind=Kind }}=Obj)
  when is_record(Kind, list_ref); Kind == null ->
    ezap_get:ref_data({list, Type}, Obj, []).
...
set_cap_table(CapTable, #builder{ pid = Pid }=Data) ->
    ok = ezap_data:set_cap_table(CapTable, Pid),
    Data.
...
dump_field(union, Object) ->
    {Tag, Value} = ezap:get(Object),
    [io_lib:format("~p = ~s", [Tag, ezap:dump(Value)])].
