...
start_capability(Module, Interfaces, Opts)
  when is_list(Interfaces), is_list(Opts) ->
    ...
    case supervisor:start_child(?SERVER, [StartArgs]) of
        {ok, Pid} ->
            Cap = #interface_ref{ owner = {ezap_capability, Pid} },
...
init([]) ->
    ...
    AChild = {capability, {ezap_capability, start_link, []},
              Restart, Shutdown, Type, [ezap_capability]},
