...
[Zap](https://zap-proto.io) library for Erlang.
...
* RPC support
  ...
  ready soon. The calculator sample from zap has been successfully
  ported to erlang.
...
In order for `ezap` to work properly, `ezap` has to be on the
Erlang lib path (i.e. `ERL_LIBS`), and `ezap/bin/zapc-erl` needs
to be on your `PATH`.
...
Try it:
    ...
    make samples
    zapc -oerl .../my_schema.zap

This will _(given that everything is working as intended)_ produce a
`.../my_schema_zap.beam` file.

A few noteworthy options you can set in the environment are:

* `EZAP_TO_ERL` - Save erlang source of compiled schema to path
  (relative to `outdir`).

* `EZAP_NO_BEAM` - Do not compile to beam code.

* `EZAP_LOAD_BEAM` - Load compiled beam code on the running
  node. Useful in case you compile files programatically, rather than
  using the zap plugin.
...
Head over to [zap-proto.dev](https://zap-proto.dev/docs/sdks/erlang) for documentation etc.